...
def preprocess_info(info_dict, player=1):
    ...
    if (self_ram_recovery == 1 and self_ram_state != 4) or self_ram_state == 12:
        self_recovery = 1
...
def disc_to_mb(disc_action, facing = 1, frame = 0):
    ...
    #special move frame by frame inputs
    if frame == 0:
        if disc_action == 16 or disc_action == 17:
            mb_array[5] = 1
        elif disc_action == 18:
            mb_array[toward] = 1
    elif frame == 1:
...
class SFRamObsDiscActWrapper(gym.Wrapper):
    ...
    def step(self, p1_action):
        ...
        for i in range(self._skip):
            obs, reward, done, info = self.env.step(disc_to_mb(p1_action,self.facing,i).tolist() + p2_action.tolist())
        ...
        return info_clean,total_reward,done,info
