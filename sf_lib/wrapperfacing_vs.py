...
def disc_to_mb(disc_action, facing = 1,):
...
class SFRamObsActWrapper(gym.Wrapper):
...
class VSDiscWrapper(gym.Wrapper):
    # takes in discrete action, turns it into a multibinary with opponent action tacked on, and returns.
    # must be run on top of subprocvecenv with no framestacking.
    ...
    def step(self,actions):
        if self.first_turn == 0:
            self.first_turn = 1
            mb_actions = np.zeros((4,24))
        else:
            p2_actions = [4]
            p2_obs = [4]
            for i in range(4):
                if self.mode == "random":
                    p2_actions[i] = MultiBinary(12).sample()
                elif self.mode == "agent":
                    p2_obs[i], _ = self.agent.predict(preprocess_info(self.last_infos[i], 2))



        obs, rewards, dones, infos = self.env.step(mb_actions)
